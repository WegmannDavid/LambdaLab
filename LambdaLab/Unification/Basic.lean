...
/-! ## Unify-step lemmas: one-shot rewrites for each branch of `unify`
on a cons, used by the `unify.induct` cases in the verification files. -/

variable {α : Type} [Signature α]

theorem unify_cons_delete (x y : α) (eqs' : Equations α) {n : Nat}
    (hxv : Signature.isVar x = some n) (hyv : Signature.isVar y = some n) :
    unify ((x, y) :: eqs') = unify eqs' := by
  rw [unify, hxv]; simp [hyv]

theorem unify_cons_occurs_l (x y : α) (eqs' : Equations α) {n : Nat}
    (hxv : Signature.isVar x = some n) (hyv : ¬ Signature.isVar y = some n)
    (hocc : Signature.occurs n y = true) :
    unify ((x, y) :: eqs') = none := by
  rw [unify, hxv]; simp [hyv, hocc]

theorem unify_cons_elim_l_some (x y : α) (eqs' : Equations α) {n : Nat}
    {rest : Unifier α}
    (hxv : Signature.isVar x = some n) (hyv : ¬ Signature.isVar y = some n)
    (hocc : ¬ Signature.occurs n y = true)
    (hrest : unify (HasSubst.single eqs' n y) = some rest) :
    unify ((x, y) :: eqs') = some ((n, y) :: rest) := by
  rw [unify, hxv]; simp [hyv, hocc, hrest]

theorem unify_cons_elim_l_none (x y : α) (eqs' : Equations α) {n : Nat}
    (hxv : Signature.isVar x = some n) (hyv : ¬ Signature.isVar y = some n)
    (hocc : ¬ Signature.occurs n y = true)
    (hnone : unify (HasSubst.single eqs' n y) = none) :
    unify ((x, y) :: eqs') = none := by
  rw [unify, hxv]; simp [hyv, hocc, hnone]

theorem unify_cons_occurs_r (x y : α) (eqs' : Equations α) {m : Nat}
    (hxv : Signature.isVar x = none) (hyv : Signature.isVar y = some m)
    (hocc : Signature.occurs m x = true) :
    unify ((x, y) :: eqs') = none := by
  rw [unify, hxv, hyv]; simp [hocc]

theorem unify_cons_elim_r_some (x y : α) (eqs' : Equations α) {m : Nat}
    {rest : Unifier α}
    (hxv : Signature.isVar x = none) (hyv : Signature.isVar y = some m)
    (hocc : ¬ Signature.occurs m x = true)
    (hrest : unify (HasSubst.single eqs' m x) = some rest) :
    unify ((x, y) :: eqs') = some ((m, x) :: rest) := by
  rw [unify, hxv, hyv]; simp [hocc, hrest]

theorem unify_cons_elim_r_none (x y : α) (eqs' : Equations α) {m : Nat}
    (hxv : Signature.isVar x = none) (hyv : Signature.isVar y = some m)
    (hocc : ¬ Signature.occurs m x = true)
    (hnone : unify (HasSubst.single eqs' m x) = none) :
    unify ((x, y) :: eqs') = none := by
  rw [unify, hxv, hyv]; simp [hocc, hnone]

theorem unify_cons_decomp (x y : α) (eqs' : Equations α) {xs : Equations α}
    (hxv : Signature.isVar x = none) (hyv : Signature.isVar y = none)
    (hd : Signature.decomp x y = some xs) :
    unify ((x, y) :: eqs') = unify (xs ++ eqs') := by
  rw [unify, hxv, hyv, hd]

theorem unify_cons_clash (x y : α) (eqs' : Equations α)
    (hxv : Signature.isVar x = none) (hyv : Signature.isVar y = none)
