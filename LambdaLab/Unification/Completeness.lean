...
If any unifier exists, the algorithm succeeds. -/
...
theorem unify_complete {α : Type} [Signature α] :
  ...
  | case3 x y eqs' m hxv hyv hocc =>
      ...
      rw [hxeq] at hxy
      have hfree : HasVars.isFree y m := (Signature.occurs_iff_isFree m y).mp hocc
      exact Signature.occurs_no_unifier y m σ hfree hyv hxy
  ...
  | case5 x y eqs' m hxv hyv hocc hnone ih =>
      ...
      have hσ_sub : σ.Unifies (HasSubst.single eqs' m y) := by
        ...
        rw [unifier_apply_absorb σ q.1 m y hxy,
            unifier_apply_absorb σ q.2 m y hxy]
        exact hq_unif
  ...
  | case6 x y eqs' hxv m hyv hocc =>
      ...
      rw [hyeq] at hxy
      have hfree : HasVars.isFree x m := (Signature.occurs_iff_isFree m x).mp hocc
      have hxv' : Signature.isVar x ≠ some m := by rw [hxv]; intro h; cases h
      exact Signature.occurs_no_unifier x m σ hfree hxv' hxy.symm
  ...
  | case8 x y eqs' hxv m hyv hocc hnone ih =>
      ...
      have hσ_sub : σ.Unifies (HasSubst.single eqs' m x) := by
        ...
        subst hqeq
        have hq_unif := hσ q (List.mem_cons_of_mem _ hq)
        rw [unifier_apply_absorb σ q.1 m x hxy',
            unifier_apply_absorb σ q.2 m x hxy']
        exact hq_unif
      exact ih σ hσ_sub hnone
  | case9 x y eqs' hxv hyv xs hdec ih =>
      ...
      have hσ' : σ.Unifies (xs ++ eqs') := by
        ...
        · exact decomp_unifier_apply_sound σ x y xs hdec hxy p hp_xs
