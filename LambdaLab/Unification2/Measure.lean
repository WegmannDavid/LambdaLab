...
theorem fresh_cons_le (p : Equation C) (eqs : Equations C) :
    fresh eqs ≤ fresh (p :: eqs) := by
  show _ ≤ max (fresh eqs) _
...
/-- Strict drop in `mvarCount` when a variable is free in the larger
    set but not the smaller. -/
