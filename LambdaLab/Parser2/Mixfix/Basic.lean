import LambdaLab.Parser2.Basic
...
**no** separator character. Every maximal separator-free run of the char stream is
one of these; nonemptiness ensures a rendered token can't vanish, so a rendered
token stream always re-splits back into the same tokens (needed for
`parse_complete`). -/
...
structure Grammar where
  ...
  **nonempty**. Any grammar that tokenizes must own a separator, so requiring one
  costs nothing — and it makes `Sep G` canonically inhabited, so the render witness
  needs no external default (`dflt`). -/
...
`Sep G` is canonically inhabited by `G.sepWitness`, so `NESep G` is always
constructible — the render witness can always promote a possibly-empty run to a
nonempty one without any external default. -/
structure NESep (G : Grammar) where
  head : Sep G
  tail : List (Sep G)

/-- Promote a possibly-empty separator run to a `NESep`, falling back to the
grammar's `sepWitness` when the run is empty. On valid input the fallback is dead
code (internal gaps always carry a separator), but it makes the render witness a
total function without any threaded default. -/
