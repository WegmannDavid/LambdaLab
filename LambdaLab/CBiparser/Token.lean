...
that `Tokenizer.lean` need not depend on the mixfix stack — a tokenizer knows nothing about
...
`abbrev Token := CBiparser.Token isSep` instead of defining a parallel type of its own.
-/
