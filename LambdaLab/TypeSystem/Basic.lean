...
All three are class parameters, with `NameAlphabet N` as an instance binder alongside, so a
...
confluent. -/
class Confluent (N Tm Ty : Type) [nameAlphabet : NameAlphabet N] extends TypeSystem N Tm Ty where
...
class LawfulHasEval (N Tm Ty : Type) [nameAlphabet : NameAlphabet N] extends
    HasEval N Tm Ty,
