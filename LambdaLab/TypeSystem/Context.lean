...
`Std.HashMap N Ty`, so both instances apply whenever the key type has a `NameAlphabet`, and they
...
`Subst 𝕊 = Std.HashMap Nat 𝕊` is untouched either way, because `Nat` has no `NameAlphabet`
...
instance (priority := low) {N Ty β : Type} [NameAlphabet N] [HasSubst Ty β] :
...
someone gives `Nat` a `NameAlphabet`, which would silently hand every *substitution* the
