import LambdaLab.ParserOld.Playground2.Uniqueness
...
With the `Level`-indexed `Tree`, a tree's index records the precedence
constraint on its root: `.tighterEq a` (an operator `a` applied), `.tighter a`
(an operand strictly tighter than `a`), or `.loosest` (a top-level expression).
A bare constant at `num` can therefore be presented at whatever level a hole
demands by supplying the matching reachability witness. -/

/-- The bare constant `n`, applied at its own node. (We pin `G := arith`:
...
theorem arith_parse_unique {tkns : List Token} {e₁ e₂ : Tree arith .loosest}
...
theorem mix_parse_unique {tkns : List Token} {e₁ e₂ : Tree mix .loosest}
