...
realization of the gaps read off it. Ported from the old `Abstraction/Tokenizer.lean`, where the
same decomposition proof discharged the `realize_complete` law that v3 demoted to this predicate.

The `Gaps` shape (a `structure` of leading run + per-token `Inner` gaps) is one presentation of the
fiber grammar `sep* (tok (sep⁺ tok)* sep*)?`; `Sketch.lean`'s two-inductive `Seperators` is
another. They present the same fibers, and `Abstraction.reindex` transports the morphism between
such presentations — the choice here is proof-reuse, not doctrine.
-/
