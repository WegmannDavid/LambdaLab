...
The indexed abstraction structure, refined against the pipeline sketch in
`Abstraction2/Example.lean` (tokenize → parse → typecheck). Supersedes the total
`Abstraction` of `Abstraction/Basic.lean`; the two coexist (separate namespaces)
until `Bicat` is ported.

Two decisions, both forced by the sketch:
