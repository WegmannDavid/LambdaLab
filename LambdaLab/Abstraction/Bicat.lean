...
/-!
# `Abs` — the `Bicategory` instance

Partiality of `abstract` costs nothing here by design: the whole 2-cell layer —
`TwoCell`, whiskerings, associator, coherence — lives on the annotation/`realize`
side and never mentions `abstract`, so `comp`'s `Option.bind` is absorbed entirely
in `Basic.lean`.

The identity 1-cell is the
`Unit`-annotated `Abstraction.id`, whose `realize` is **definitionally** the index —
a subtype fibre `{a' // a' = a}` would pin the index only propositionally, which
is what forces a transport in the right unitor. Here `rightUnitor` is transport-free and
