...
theorem List.nodup_distinct {β : Type} [DecidableEq β] (l : List β) : l.distinct.Nodup := by
  induction l with
...
def Equations.vars {α : Type} [Signature α] (eqs : Equations α) : List Nat :=
...
/-- A variable at or above `fresh eqs` is never free in `eqs` — the
    contrapositive of `fresh_gt_free`. Not used by the measure; kept for the elaborator's
    pruning arguments. -/
...
theorem mem_vars_iff_isFree (eqs : Equations α) (n : Nat) :
...
theorem mem_distinct_vars_iff_isFree (eqs : Equations α) (n : Nat) :
    n ∈ eqs.vars.distinct ↔ HasVars.isFree eqs n :=
