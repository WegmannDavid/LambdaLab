...
mutual
  ...
  def tryOp {α : Type} (g : Grammar α) (input : List String) (op : Operator α) :
      Option (Tree α × { rest : List String // rest.length < input.length }) :=
  ...
  def tryTail {α : Type} (g : Grammar α) (head : Tree α) (input : List String)
      (op : Operator α) :
      Option (Tree α × { rest : List String // rest.length < input.length }) :=
  ...
  def tryTailLoop {α : Type} (g : Grammar α) (head : Tree α) (input : List String) :
    ...
    | some (newHead, ⟨rest, hR⟩) =>
        ...
        ⟨next.fst, ⟨next.snd.val, by have := next.snd.property; omega⟩⟩
    | none =>
        -- Juxtaposition: only fires if enabled in `g` and the next
        -- token can start an expression (so that e.g. `)` or `,` aren't
        -- swallowed).
  ...
  def parseTreeClosedSeq {α : Type} (g : Grammar α) :
      List String → ∀ (hs : List (HoleSpec α)), ∀ (input : List String),
        Option (Children α hs × { rest : List String // rest.length < input.length })
    | [last],    [],                input =>
...
def parseAllTree {α : Type} (g : Grammar α) (input : List String) : Option (Tree α) :=
  match parseTree g input with
  | some (t, ⟨[], _⟩) => some t
  | _                 => none
