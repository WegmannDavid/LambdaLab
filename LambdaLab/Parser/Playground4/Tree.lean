import LambdaLab.Parser.Playground4.Basic
...
def Level.nextChild (op : G.Op) : List (Level G) :=
  match (G.operator op) with
  | ⟨ _, .const tkn ⟩    => sorry
  | ⟨ _, .closed tkn o ⟩ => sorry
  | ⟨ _, .prefx tkn, ⟩   => sorry
  | ⟨ _, .infx o ⟩       => sorry
  | ⟨ _, .postfx o ⟩     => sorry
...
mutual
  ...
  | op : (o : G.Op) → Level.condition l o → Children G (G.operator o) → Expr G l

  inductive Children (G : Grammar) : (Σ f, Operator f) → Type where
  | nil : Children G nil
...
mutual
  def Expr.flatten (e : Expr G l) : List Token :=
    match e with
    |.op _ _ cs => cs.flatten

  def Children.flatten (cs : Children G l) : List Token :=
    match cs with
    | .nil => []
    | .cons c cs => c.flatten ++ cs.flatten
end
