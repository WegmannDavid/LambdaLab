...
/-- `TighterEq` is transitive — composing two tighter-paths. -/
...
/-- A non-trivial tighter-eq path is a strict tighter-path. -/
theorem TighterEq.toTighter {Op : Type} {t : Op → List Op} {a c : Op}
    (h : TighterEq t a c) (hne : a ≠ c) : Tighter t a c := by
  induction h with
  | refl => exact absurd rfl hne
  | @step a b c hmem h' ih =>
      by_cases hbc : b = c
      · subst hbc; exact Tighter.base hmem
      · exact Tighter.step hmem (ih hbc)

/-- Peel the first step of a strict tighter-path. -/
theorem Tighter.split {Op : Type} {t : Op → List Op} {a c : Op} (h : Tighter t a c) :
    ∃ b, b ∈ t a ∧ TighterEq t b c := by
  cases h with
  | base hmem => exact ⟨_, hmem, TighterEq.refl⟩
  | step hmem h' => exact ⟨_, hmem, h'.toTighterEq⟩

/-- An immediate step `b ∈ t a` followed by a tighter-eq path to `c` is a strict
tighter-path `a` ⟶ `c`. -/
theorem tighter_of_mem_tighterEq {Op : Type} {t : Op → List Op} {a b c : Op}
    (hb : b ∈ t a) (h : TighterEq t b c) : Tighter t a c := by
  by_cases hbc : b = c
  · subst hbc; exact Tighter.base hb
  · exact Tighter.step hb (h.toTighter hbc)

/-- A strict tighter-path extended by a tighter-eq path stays strict. -/
theorem Tighter.trans_tighterEq {Op : Type} {t : Op → List Op} {a c b : Op}
    (h : Tighter t a c) (h' : TighterEq t c b) : Tighter t a b := by
  obtain ⟨d, hd, hdc⟩ := h.split
  exact tighter_of_mem_tighterEq hd (hdc.trans h')
