...
via `grind`. -/
theorem unifyList_unifies {A α : Type} [Atom A] [Signature A α] :
...
/-! ## Public, `Subst A α`-form soundness. -/

/-- A `Subst A α` unifies a set of equations in parallel form. -/
abbrev Subst.Unifies {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (eqs : Equations α) : Prop :=
  ∀ p ∈ eqs, HasSubst.pSubst p.1 σ = HasSubst.pSubst p.2 σ

/-- **Soundness of `unify`.** When `unify eqs` returns `some σ`, the
returned `Subst A α` makes every equation hold under parallel substitution.
Corollary of `unifyList_unifies` via `Unifier.apply_eq_pSubst_toSubst`. -/
theorem unify_unifies {A α : Type} [Atom A] [Signature A α] :
