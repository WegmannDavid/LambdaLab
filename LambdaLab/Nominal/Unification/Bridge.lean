...
theorem pSubst_var_eq (n : A) (s : α) :
    pSubst (var n) ((∅ : Subst A α).insert n s) = s := by
  rw [pSubst_var]
...
theorem single_var_self (t : α) (n : A) :
    HasSubst.single t n (var n) = t := by
  show pSubst t ((∅ : Subst A α).insert n (var n)) = t
...
theorem single_off (t : α) (n : A) (s : α)
    (hf : ¬ HasVars.isFree t n) :
    HasSubst.single t n s = t := by
  show pSubst t ((∅ : Subst A α).insert n s) = t
  induction t using term_ind with
  | var_case m =>
      rw [pSubst_var, Std.HashMap.getD_insert]
      have hnm : n ≠ m := by
...
theorem pSubst_empty (t : α) : pSubst t (∅ : Subst A α) = t := by
  induction t using term_ind with
  | var_case n =>
...
theorem pSubst_ground {t : α} (σ : Subst A α) (h : HasVars.Ground t) : pSubst t σ = t := by
  induction t using term_ind with
...
instance instGroundStable : GroundStable A α α where
...
theorem pSubst_single_compose (t : α) (n : A) (s : α) (σ : Subst A α) :
    pSubst (HasSubst.single t n s) σ
      = pSubst t (σ.insert n (pSubst s σ)) := by
  show pSubst (pSubst t ((∅ : Subst A α).insert n s)) σ
  ...
  | construct_case c args ih =>
      ...
      show pSubst ((Vector.ofFn (fun j : Fin (arity c) =>
                  pSubst (args.get j) ((∅ : Subst A α).insert n s))).get ⟨i, hi⟩) σ
            = pSubst (args.get ⟨i, hi⟩) (σ.insert n (pSubst s σ))
      have hget' : (Vector.ofFn (fun j : Fin (arity c) =>
                  pSubst (args.get j) ((∅ : Subst A α).insert n s))).get ⟨i, hi⟩ =
                pSubst (args.get ⟨i, hi⟩) ((∅ : Subst A α).insert n s) := by
...
theorem isFree_var (n m : A) :
    HasVars.isFree (var n : α) m ↔ m = n :=
  var_isFree n m

theorem isFree_construct (c : Constructor α) (args : Vector α (arity c)) (m : A) :
...
theorem decomp_isFree (x y : α) (xs : Equations α) (n : A)
...
theorem decomp_single (x y : α) (xs : Equations α) (n : A) (s : α)
    (hd : decomp x y = some xs) :
  ...
  subst hxs
  rw [hxc, hyc]
  show decomp (pSubst _ ((∅ : Subst A α).insert n s))
              (pSubst _ ((∅ : Subst A α).insert n s)) = _
  ...
  have hget : ∀ a : Vector α (arity c),
      (Vector.ofFn (fun j : Fin (arity c) =>
          pSubst (a.get j) ((∅ : Subst A α).insert n s))).get i =
        pSubst (a.get i) ((∅ : Subst A α).insert n s) := fun _ => by
...
injectivity. -/
theorem apply_construct (u : Unifier A α) (c : Signature.Constructor α)
  ...
  | cons p rest ih =>
      ...
      show (Vector.ofFn (fun j : Fin (Signature.arity c) =>
              Signature.pSubst (args.get j) ((∅ : Subst A α).insert n s))).get ⟨i, hi⟩ =
...
`toSubst` conversion compresses the list into one parallel `Subst A α` by
...
`unify` API return `Option (Subst A α)` while the internal proofs continue
to work with the list form. -/

theorem apply_eq_pSubst_toSubst (u : Unifier A α) (t : α) :
...
theorem Signature.decomp_apply_sound {A α : Type} [Atom A] [Signature A α] :
    ∀ (l : Unifier A α) (x y : α) (xs : Equations α),
...
theorem Signature.hasSubst_pSubst_eq {A α : Type} [Atom A] [Signature A α] (t : α) (σ : Subst A α) :
...
theorem Signature.decomp_unifier_sound {A α : Type} [Atom A] [Signature A α]
    (x y : α) (xs : Equations α) (σ : Subst A α)
...
private theorem isVar_none_construct {A α : Type} [Atom A] [Signature A α] {t : α}
...
theorem Signature.decomp_none_no_unifier {A α : Type} [Atom A] [Signature A α]
    (x y : α) (σ : Subst A α)
...
rules of `unify`. -/
theorem Signature.unifier_absorb {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (t : α) (n : A) (s : α)
  ...
  show Signature.pSubst (Signature.pSubst t ((∅ : Subst A α).insert n s)) σ = Signature.pSubst t σ
  ...
  | construct_case c args ih =>
      ...
      have hget1 : (Vector.ofFn (fun j : Fin (Signature.arity c) =>
                      Signature.pSubst (args.get j) ((∅ : Subst A α).insert n s))).get i =
                   Signature.pSubst (args.get i) ((∅ : Subst A α).insert n s) := by
        show ((Vector.ofFn _)[i.val]'i.isLt) = _; simp
...
arguments. -/
private theorem size_pSubst_construct {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (c : Signature.Constructor α)
    (args : Vector α (Signature.arity c)) :
...
private theorem size_pSubst_le_of_isFree {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (t : α) (n : A) (h : HasVars.isFree t n) :
...
theorem Signature.occurs_no_unifier {A α : Type} [Atom A] [Signature A α]
    (t : α) (n : A) (σ : Subst A α)
    (hf : HasVars.isFree t n)
...
theorem Unifier.toSubst_cons_pSubst {A α : Type} [Atom A] [Signature A α] (n : A) (s : α)
...
theorem Signature.isFree_pSubst {A α : Type} [Atom A] [Signature A α] (t : α) (σ : Subst A α) (m : A) :
...
theorem Signature.pSubst_comp {A α : Type} [Atom A] [Signature A α]
    (σ τ : Subst A α) (t : α) :
...
instance instLawfulComp {A α : Type} [Atom A] [Signature A α] : LawfulComp A α α where
...
theorem MoreGeneral.refl {A α : Type} [Atom A] [Signature A α] (σ : Subst A α) :
    MoreGeneral σ σ :=
  ⟨(∅ : Subst A α),
   fun t => (Signature.pSubst_empty (HasSubst.pSubst t σ)).symm⟩

/-- Transitivity of `MoreGeneral`: chains compose via `Subst.comp`. -/
theorem MoreGeneral.trans {A α : Type} [Atom A] [Signature A α] {σ₁ σ₂ σ₃ : Subst A α}
...
/-- If every key of σ lies in `s`, restricting to `s` changes nothing, on any term at all. -/
theorem Subst.restrictTo_action_eq {A α : Type} [Atom A] [Signature A α] (σ : Subst A α)
    (s : List A) (hkeys : ∀ k, σ.get? k ≠ none → k ∈ s) (t : α) :
    HasSubst.pSubst t (Subst.restrictTo σ s) = HasSubst.pSubst t σ := by
...
/-- Restricting σ to the atom set `s` doesn't change pSubst's action on any `t` all of whose free
atoms lie in `s`. The elaborator's pruning lemma: a computed σ carries bindings at the internal
atoms the algorithm drew, those are invisible to the source, so dropping them gives an
action-equivalent substitution on the source. -/
theorem Signature.pSubst_restrictTo {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (s : List A) (t : α)
    (h_supp : ∀ a, HasVars.isFree t a → a ∈ s) :
    HasSubst.pSubst t (Subst.restrictTo σ s) = HasSubst.pSubst t σ := by
  show Signature.pSubst t (Subst.restrictTo σ s) = Signature.pSubst t σ
  ...
  | _ k ih =>
    ...
    | Sum.inl m =>
        ...
        have ht : t = Signature.var m := by
          ...
          exact this.symm
        subst ht
        have hmn : m ∈ s := h_supp m ((Signature.isFree_var m m).mpr rfl)
        rw [Signature.pSubst_var, Signature.pSubst_var]
        rw [Std.HashMap.getD_eq_getD_getElem?, ← Std.HashMap.get?_eq_getElem?,
            Std.HashMap.getD_eq_getD_getElem?, ← Std.HashMap.get?_eq_getElem?,
            Subst.restrictTo_get?, if_pos hmn]
    | Sum.inr ⟨c, args⟩ =>
        ...
        exact fun a ha => h_supp a ((Signature.isFree_construct c args a).mpr ⟨i, ha⟩)
...
instance instLawfulRestrict {A α : Type} [Atom A] [Signature A α] : LawfulRestrict A α α where
  pSubst_restrictTo := fun t σ s h => Signature.pSubst_restrictTo σ s t h

theorem Signature.pSubst_insert_fresh {A α : Type} [Atom A] [Signature A α]
    (σ : Subst A α) (k : A) (v : α) (t : α)
